...
local add_value = function(cuurent, new)
...
opt_mt = {
    ...
    __add = function(left, right)
        ...
        -- For some reason passing values here make them nil.
        -- Therefore, we just paste the function below
        ...
        if type(right) == 'string' then
            if vim.tbl_islist(current) then
                ...
                set_key_value(current, right)
            end

            left._value = current
        elseif type(right) == 'table' then
            if vim.tbl_islist(current) then
                assert(vim.tbl_islist(right))
                vim.list_extend(current, right)
            else
                assert(not vim.tbl_islist(right), vim.inspect(right) .. vim.inspect(current))
                current = vim.tbl_extend("force", current, right)
            end
